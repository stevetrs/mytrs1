...
def is_chinese(char):
    if '\u4e00' <= char <= '\u9fff':
        ...
        return False
def ifisallnum(x,special,chinese):#如果全是字母,数字和特殊符号，返回0，否则返回原字符串
    nums=special
    res=x
    if chinese:
        for i in x:
...
if config['cleaner']:
    ...
    for j in range(0, len(target) - 1):
        if not ifisallnum(target.loc[j, 'src'],config['special'],config['chinese']):target.drop([j],axis=0,inplace=True)
#按文本从长到短排序,长文本优先翻译
