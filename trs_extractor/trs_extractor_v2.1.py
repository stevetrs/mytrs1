...
# 返回替换好的json文件
def readf(untrs_data, trsed_data, same):
        ...
        if tp == tp2:
            if tp == list:
                ...
                # 长度不等
                    ...
                    if readtype(trsed_data) and config['difflist']:
                        pa = True
                        temp=trsed_data
                        # for iii in trsed_data:
                        #     temp.append(readf(iii, False, False))
                    else:same=False
                if not pa:
                ...
                for i in untrs_data.keys():
                    if i in trsed_data.keys():
                        temp[i] = readf(untrs_data[i], trsed_data[i], same)
                    else:
                        temp[i] = readf(untrs_data[i], False, same)
            else:
        ...
        if name in trsed_names:
            ...
            try:
                ...
                with open(untrs_path+name,'r', encoding='utf-8-sig') as f:
                    sig=True
                    untrs_data = json.load(f)
            data=readf(untrs_data,trsed_data,True)
